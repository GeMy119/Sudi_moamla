...
@Component({
  ...
  styleUrl: './nationality-search.component.css',
})
export class NationalitySearchComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.loading = true;
    this.nationalityService.search(this.identityNumber.trim(), this.sourceNumber.trim()).subscribe({
      next: () => {
        this.loading = false;
        // بننقل لصفحة النتيجة، ورقم الهوية والصادر بيتبعتوا في الـ URL عشان لو المستخدم عمل refresh
        this.router.navigate(['/nationality-inquiry/result'], {
          queryParams: {
