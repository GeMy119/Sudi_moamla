...
@Component({
  selector: 'app-iqama-search',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './iqama-search.component.html',
  styleUrl: './iqama-search.component.css',
})
export class IqamaSearchComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.iqamaService.search(this.identityNumber.trim(), this.sourceNumber.trim()).subscribe({
      next: () => {
        this.loading = false;
        this.router.navigate(['/iqama-inquiry/result'], {
          queryParams: {
